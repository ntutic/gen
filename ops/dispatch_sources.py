...
line; the script parses it and flips the spider file's `enabled` flag,
saving notes per source under the log dir. A retryable FAILED verdict runs
...
Enablement lives in the spider file (`enabled = False/True`); the database
mirrors it on sync. Builders always write `enabled = False` and never flip
it. Mechanical rule: the flag flips to True only when the evaluator verdict
is "succeeded" AND the local gate (worker exit 0, spider/test files
present, name matches, tests pass) holds. Anything else leaves the spider
file untouched. An applied enable mechanically enqueues one production run
for it (`python -m scrapectl enqueue SOURCE`).
...
- Spider `name` and filename must both be `{source_id}`. Set
  `source_kind`, `key_description`, `start_urls`, and `notes`, and write an
  explicit `enabled = False` class attribute. Never set it to True;
  enabling is applied mechanically from the reviewer's verdict after your
  build (the database mirrors the spider file's flag on sync).
...
def spider_enabled(text: str) -> bool | None:
    """Spider file's `enabled` class attribute; None when the line is missing."""
    matches = re.findall(r"(?m)^\s*enabled\s*=\s*(True|False)\s*(?:#.*)?$", text)
    if not matches:
        return None
...
def spider_status(source_id: str) -> dict:
    """Filesystem facts about worker output; never runs the spider."""
    ...
    if spider_path.is_file():
        ...
        info["enabled"] = spider_enabled(text)
    return info
...
def reconcile_source(source_id: str, log_dir: Path, test_runner=run_source_tests) -> dict:
    ...
    if not status["spider_exists"]:
        ...
        if status["enabled"] is True:
            reasons.append("spider enabled in file without verified completeness")
        elif status["enabled"] is None:
            reasons.append("spider has no explicit enabled flag")
...
3. Review against the repo rules: name == filename == source ID; explicit
   ...
   key_description, and notes updated; explicit `enabled = False` still in
   place. Never set the enabled flag yourself: the pipeline applies your
   verdict mechanically after this review. Judge only; do not act.
...
def apply_decision(source_id: str, verdict: dict | None, local_ready: bool, local_reasons: list[str]) -> dict:
    """Mechanically apply succeeded/blocked/failed via the spider file's flag.

    The only file edit is a strict one-line flip of `enabled = False` to
    `enabled = True` in the spider file; the database mirrors it on sync.
    """
    if verdict is None:
    ...
    try:
        ...
        return {"applied": False, "detail": f"enable failed: cannot read spider file: {exc}"}
    flipped, count = re.subn(r"(?m)^(\s*enabled\s*=\s*)False(\s*(?:#.*)?)$", r"\1True\2", text)
    if count != 1:
        return {"applied": False, "detail": f"enable failed: expected one `enabled = False` line, found {count}"}
    try:
        spider_path.write_text(flipped, encoding="utf-8")
    except OSError as exc:
        return {"applied": False, "detail": f"enable failed: cannot write spider file: {exc}"}
    return {"applied": True, "detail": "verdict succeeded + local gate passed; spider enabled = True"}
