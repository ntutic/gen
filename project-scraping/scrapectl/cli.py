...
def parser() -> argparse.ArgumentParser:
    ...
    enabled = commands.add_parser("enabled", help="Print whether a scraper is enabled in the database")
    enabled.add_argument("scraper_id")
    ...
    serve_admin = commands.add_parser("serve-admin", help="Serve administration on loopback")
    serve_admin.add_argument("--port", type=int, default=8001)
    return root
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "scrapers":
        ...
        return 0
    if args.command in ("enable", "disable", "enabled"):
        with Session() as session:
            row = session.get(Scraper, args.scraper_id)
            if row is None:
                raise LookupError(f"scraper {args.scraper_id!r} does not exist")
            if args.command in ("enable", "disable"):
                row.enabled = args.command == "enable"
                session.commit()
            print(json.dumps({"id": row.id, "enabled": row.enabled}))
        return 0
    ...
    if args.command == "serve":
        uvicorn.run("scrapectl.api:app", host=args.host, port=args.port)
        return 0
    if args.command == "serve-admin":
        uvicorn.run("scrapectl.admin:app", host="127.0.0.1", port=args.port)
        return 0
