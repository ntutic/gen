...
def sync_scrapers(session):
    ...
    for name, spider in spiders.items():
        ...
        if row is None:
            # The database owns the enabled flag: new scrapers register
            # disabled and only an explicit enable flips them.
            row = Scraper(id=name, source_id=name, module=spider.__module__, enabled=False)
            session.add(row)
